...
class consejeriaController extends Controller {    
    ...
    {if (!isset($_GET['p'])){$_GET['p']=1;}
        ...
        if($_SESSION['cargo']=='Presidente' && $_SESSION['comicion']=='COMISION ESPECIAL DE TUTORIA'){$data['grilla'] = $this->grilla("consejeria",$cols, $data['data']['rows'],$opt,$data['pag'],TRUE,TRUEs,false,false,false,false,$presidente); }
 else {
    $data['grilla'] = $this->grilla("consejeria",$cols, $data['data']['rows'],$opt,$data['pag'],true,true,false,true,false,false);
...
//        $dep=$this->mostrar_Facultad_idUsusario($_SESSION['idusuario']);
        if($_SESSION['idperil2']==4){$WHERE=" where tipo_evento.idtipo_evento= '2'"; $code='1';$label='Consejeria';} ;
        if(isset($_GET['modo_sin_cargo'])){$code="2";$label='Consejeria';}
        $data['tipo_evento']="<input type='hidden' id='idtipo_evento' name='idtipo_evento' value='2'><strong>Consejeria</strong>";  
        $view->setData($data);
    ...
    public function save(){ 
        ...
        if ($_POST['idevento']=='') {
            ...
            $p = $obj->update($_POST,$_SESSION['idusuario']);
            if ($p[0]){
                header('Location: index.php?controller=consejeria');
            } else {
            $data = array();
